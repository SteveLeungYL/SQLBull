...
container_home_directory = "/home/duckdb/"
...
def pull_docker_results_to_local_fs(container_name: str):
    ...
    command = f"docker cp {container_name}:{container_home_directory}/fuzzing/Bug_Analysis/ {dest_loc}/Bug_Analysis"
    ...
    command = f"docker cp {container_name}:{container_home_directory}/duckdb_version {dest_loc}/duckdb_version"

    print(f"Running {command}\n")
    subprocess.run([command], shell=True,
                   stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

    command = f"docker cp {container_name}:{container_home_directory}/fuzzer_version {dest_loc}/fuzzer_version"
