...
#include "../query_plan/query_plan_handler.h"
...
static u_int32_t query_plan_hash = 0;
static set<u_int32_t> query_plan_set;
...
static inline u8 has_new_bits(u8 *virgin_map, const string cur_seed_str = "") {
  ...
  u32 i = (MAP_SIZE >> 2);
    ...
    // override
    if (query_plan_set.count(query_plan_hash) != 0) {
        return 0;
    } else {
        query_plan_set.insert(query_plan_hash);
        return 2;
    }
...
static u8 fuzz_one(char **argv) {
    ...
    for (int stmt_idx = 0; stmt_idx < 30; stmt_idx++) {
        ...
        if (p_oracle->is_oracle_select_stmt(cur_stmt_str)) {
            ...
            vector<string> v_query_plan = retrieve_query_plan(whole_query_seq_with_next, g_cockroach_output);
            query_plan_hash = hash_v_query_plan(v_query_plan);
            ...
            string whole_query_seq_no_opt =
                    no_opt_sql_str + whole_query_seq + explain_prefix + cur_stmt_str + ";\n";
...
EXP_ST void setup_dirs_fds(void) {
    ...
    fprintf(
            ...
            "num_grammar_edge_cov,query_plan_size"
            "\n");
