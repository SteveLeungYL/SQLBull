...
uint32_t hash_v_query_plan(std::vector<string> const& vec_in) {

    vector<uint32_t> vec;
    vec.reserve(vec_in.size()+1);

    for (const string& cur_str : vec_in) {
        vec.push_back(uint32_t(fuzzing_hash(cur_str.c_str(), (int)cur_str.size())));
    }
